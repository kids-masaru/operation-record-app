...
if st.session_state['show_settings']:
    ...
    # Use session state to persist creds if uploaded previously
    creds_file = None # Re-eval if hidden? No, file_uploader is widget. 
    # Streamlit file_uploader resets if unmounted. 
    # To keep it persistent while hiding, we shouldn't unmount the widget.
    # So we'll keep the uploader "visible" but maybe in an expander that defaults to closed if we want it hidden?
    # User asked for "Hidden behind gear", so let's put it in an expander that is toggled by the button state, 
    # OR we use sidebar which is "hidden" by default. 
    # Let's try sidebar for the "Gear" effect if we want persistence easily, 
    # BUT user said "Gear icon to reveal".
    pass

# To handle File Uploader Persistence with Custom Toggle, it's tricky in pure Streamlit.
# Strategy: Puts settings in the Sidebar, and the "Gear" button just opens the sidebar programmatically?
# No, Streamlit sidebar control is limited.
# Alternative: Always render the uploader but use CSS to hide it? No security.
# Best: Just put the uploader in the main flow but inside an `if st.session_state['show_settings']:` block.
# Note: If user closes settings, file *might* get cleared. 
# workaround: Warn user or just keep it open once set.

# Let's put the CREDENTIALS in a Sidebar that matches the "Gear" metaphor.
# Actually, the user's request "Settings hidden by gear, press to see" fits st.sidebar perfectly if we just style the sidebar toggle?
# Or we just use a collapsed sidebar and the gear button is the default.

# Let's stick to the center logic for the main app.

if st.session_state['show_settings']:
    pass # Already rendered above
else:
    # Need to access creds_file even if hidden? 
    # If hidden, widget doesn't verify. 
    # We will assume user sets it up once.
    # Let's try using st.expander which is "Hidden" visually but always in DOM?
    # User said "Push gear to see".
    pass

# --- Redoing Settings Logic for Persistence ---
# We will place settings in an st.expander that is controlled by state, BUT specific to this app 
# let's just use a standard expander styled delicately if we can't do modal easily.
# actually, let's use the sidebar, but we will make it accessible via the gear logic if possible.
# Simplified: Put settings in an Expander at the top, labeled "⚙️ 設定", default collapsed.
# But user said "Gear icon to hide it".
# Let's try: 
# Main View 
...
st.markdown('<div class="app-title">Nursery Sync</div>', unsafe_allow_html=True)
